...
void peer_manager_on_data_recv(const esp_now_recv_info_t *recv_info, const uint8_t *data, int data_len)
{
    ...
    // 3) Solo JSON (si no es JSON, se ignora)
    const char *p = buffer;
    while (*p == ' ' || *p == '\t')
        p++;
    if (*p != '{')
    {
        ESP_LOGW(TAG, "Payload no-JSON; se ignora.");
    ...
    // 5) Guardar en NVS y ACK
    esp_err_t se = peer_manager_save_irrigation_config(&cfg, recv_info->src_addr);
    if (se == ESP_OK)
    {
        ESP_LOGI(TAG, "Configuración (JSON) guardada en NVS. Enviando ACK...");
        ...
        const char *ack = "CFG_OK";
        if (s_cfg_ready_sem) xSemaphoreGive(s_cfg_ready_sem);
        esp_err_t es = esp_now_send(recv_info->src_addr, (const uint8_t *)ack, strlen(ack));
        if (es != ESP_OK)
        {
            ESP_LOGW(TAG, "Falló envío de ACK: %s", esp_err_to_name(es));
        }
        else
        {
